...
static void kernel_pkcorr_G3(const int *ndim, const double xx[],const int *ncomp, double ff[])
{
...
void calc_pkcorr_from_gamma3_tree(FLAG a_, FLAG b_, histo_t k_, histo_t *pkcorr_G3_tree)
{
	...
	Vegas(NDIM, NCOMP, kernel_pkcorr_G3, EPSREL, EPSABS, VERBOSE, MINEVAL, MAXEVAL, NSTART, NINCREASE, &neval, &fail, integral, error, prob);
...
static int kernel_pkcorr_G3(const int *ndim, const cubareal xx[],const int *ncomp, cubareal ff[], void *userdata)
{
#define f ff[0]
	f = 0.;
}
...
void calc_pkcorr_from_Gamma3(FLAG a, FLAG b, histo_t k, histo_t *pkcorr_G3_tree)
{
	...
	Vegas(NDIM, NCOMP, kernel_pkcorr_G3, USERDATA, NVEC, EPSREL, EPSABS, VERBOSE, SEED, MINEVAL, MAXEVAL, NSTART, NINCREASE, NBATCH, GRIDNO, STATEFILE, SM_PIN, &neval, &fail, integral, error, prob);
