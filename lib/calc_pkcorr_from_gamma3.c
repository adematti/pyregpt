...
static histo_t k,kmin,kmax,logkmin,logkmax;
static size_t a,b;
#pragma omp threadprivate(k,kmin,kmax,logkmin,logkmax,a,b)
...
static void integ_pkcorr_G3(const int *ndim, const double xx[],const int *ncomp, double ff[])
{
	...
	histo_t p = my_exp(logkmin + (logkmax - logkmin) * ((histo_t) xx[0]));
	histo_t theta1 = ((histo_t) xx[1]) * M_PI;
	histo_t phi1 = ((histo_t) xx[2]) * 2. * M_PI;
	histo_t q = my_exp(logkmin + (logkmax - logkmin) * ((histo_t) xx[3]));
	...
	if ((k_p_q>=kmin)&&(k_p_q<=kmax)&&(prod_q_kp<=0.5*k_p*k_p)) {
		...
		histo_t jacobian = (logkmax-logkmin) * M_PI * 2. * M_PI;
