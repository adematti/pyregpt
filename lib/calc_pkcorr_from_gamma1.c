...
static size_t nq_1loop = 600;
static size_t nq_2loop = 500;
static INTERPOL interpol_q_1loop = POLY;
static INTERPOL interpol_q_2loop = POLY;
...
{
	nq_2loop = nq_2loop_;
	interpol_q_2loop = get_interpol(interpol_q_2loop_);
}

void init_gamma1()
{
	init_gauss_legendre_q(&gauss_legendre_q_1loop,nq_1loop,interpol_q_1loop,-1,-1);
	init_gauss_legendre_q(&gauss_legendre_q_2loop,nq_2loop,interpol_q_2loop,-1,-1);
}
