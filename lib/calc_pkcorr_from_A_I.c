...
void fp_A(histo_t x, histo_t mu, histo_t* kernel_fp_A, histo_t* kernel_fp_tA)
{
	kernel_fp_A[0] = x*mu;
	kernel_fp_A[1] = - x*x * (3.*x*mu-2.) * (mu*mu-1.) / (1.+x*x-2.*mu*x) /2.;
	kernel_fp_A[2] = x*( 2.*mu + x*(2.-6.*mu*mu) + x*x*mu*(-3.+5.*mu*mu) ) / (1.+x*x-2.*mu*x) / 2.;
	kernel_fp_A[3] = kernel_fp_A[0];
	kernel_fp_A[4] = kernel_fp_A[1];
	kernel_fp_A[5] = kernel_fp_A[2];
	
	kernel_fp_tA[0] = - x*x * (x*mu-1.) / (1.+x*x-2.*x*mu);
	kernel_fp_tA[1] = x*x * (3.*x*mu-1.) * (mu*mu-1.) / (1.+x*x-2.*x*mu) /2.;
	kernel_fp_tA[2] = x*x * (-1. + 3.*x*mu + 3.*mu*mu - 5.*x*mu*mu*mu ) / (1.+x*x-2.*x*mu) /2.;
	kernel_fp_tA[3] = kernel_fp_tA[0];
	kernel_fp_tA[4] = kernel_fp_tA[1];
	kernel_fp_tA[5] = kernel_fp_tA[2];
}
...
static void kernel_A_tA_2loop_I(size_t iq, histo_t mu, histo_t* kernel_A_tA)
{
	...
	kernel_A_tA[0] = kernel_fp_A[0] * b211A * x;
	kernel_A_tA[1] = kernel_fp_A[1] * b221A * x;
	kernel_A_tA[2] = kernel_fp_A[2] * b221A * x;
	kernel_A_tA[3] = kernel_fp_A[3] * b212A * x;
	kernel_A_tA[4] = kernel_fp_A[4] * b222A * x;
	kernel_A_tA[5] = kernel_fp_A[5] * b222A * x;

	kernel_A_tA[6] = kernel_fp_tA[0] * b211tA * x;
	kernel_A_tA[7] = kernel_fp_tA[1] * b221tA * x;
	kernel_A_tA[8] = kernel_fp_tA[2] * b221tA * x;
	kernel_A_tA[9] = kernel_fp_tA[3] * b212tA * x;
	kernel_A_tA[10] = kernel_fp_tA[4] * b222tA * x;
	kernel_A_tA[11] = kernel_fp_tA[5] * b222tA * x;
}
...
void calc_pkcorr_from_A_2loop_I(histo_t k,histo_t* pkcorr_A)
{
	...
	for (iq=0;iq<nq;iq++) {
		...
		for (imu=0;imu<nmu;imu++) {
			kernel_A_tA_2loop_I(iq, gauss_legendre_mu.mu[imu], kernel);
