...
static FLAG a,b;
static INTERPOL interpol_pk = POLY;
...
void set_precision_pk_lin(char* interpol_pk_lin_)
{
	interpol_pk = get_interpol(interpol_pk_lin_);
}
...
void run_2loop(char* a_,char* b_,size_t num_threads)
{
	...
	timer(0);
	find_pk_lin(terms_2loop.k,terms_2loop.pk_lin,terms_2loop.nk,interpol_pk);
...
#pragma omp for nowait schedule(dynamic)
		for (ik=0;ik<terms_2loop.nk;ik++) {
			...
			calc_running_sigma_v2(k,&(terms_2loop.sigma_v2[ik]));
...
void set_k_bias(size_t nk,histo_t* k,histo_t* pk_lin,histo_t* pkbias_b2d,histo_t* pkbias_bs2d,histo_t* pkbias_b2t,histo_t* pkbias_bs2t,histo_t* pkbias_b22,histo_t* pkbias_b2s2,histo_t* pkbias_bs22,histo_t* sigma3sq)
{
	terms_bias.nk = nk;
	terms_bias.k = k;
...
void run_bias(size_t num_threads)
{
	...
	timer(0);
	find_pk_lin(terms_bias.k,terms_bias.pk_lin,terms_bias.nk,interpol_pk);
...
#pragma omp for nowait schedule(dynamic)
		for (ik=0;ik<terms_bias.nk;ik++) {
			...
			if (ik % step_verbose == 0) printf(" - Computation done at %zu percent.\n",ik*STEP_VERBOSE/step_verbose);
#endif //_VERBOSE
			terms_bias.pkbias_b2d[ik] = calc_pkcorr_from_bias(DELTA,k,kernel_b2);
			terms_bias.pkbias_bs2d[ik] = calc_pkcorr_from_bias(DELTA,k,kernel_bs2);
			terms_bias.pkbias_b2t[ik] = calc_pkcorr_from_bias(THETA,k,kernel_b2);
			terms_bias.pkbias_bs2t[ik] = calc_pkcorr_from_bias(THETA,k,kernel_bs2);
			terms_bias.pkbias_b22[ik] = calc_pkcorr_from_bias(DELTA,k,kernel_b22);
			terms_bias.pkbias_b2s2[ik] = calc_pkcorr_from_bias(DELTA,k,kernel_b2s2);
			terms_bias.pkbias_bs22[ik] = calc_pkcorr_from_bias(DELTA,k,kernel_bs22);
			terms_bias.sigma3sq[ik] = calc_pkcorr_from_bias(DELTA,k,kernel_sigma3sq);
		}
#pragma omp critical
		{
			free_bias();
...
void set_k_A_B(size_t nk,histo_t* k,histo_t* pk_lin,histo_t* A,histo_t* B)
{
	terms_A_B.nk = nk;
	terms_A_B.k = k;
...
void run_A_B(size_t num_threads)
{
	...
	timer(0);
	find_pk_lin(terms_A_B.k,terms_A_B.pk_lin,terms_A_B.nk,interpol_pk);
