...
histo_t WFunc(histo_t k1, histo_t k2, histo_t k3, histo_t q)
{
	...
	//if (isnan(my_log((aa-bb)/(aa+bb)))) printf("%.3lf ",bb);
...
histo_t A_mat(size_t m,size_t n,histo_t* r,histo_t* x)
{
	...
	if ((m==1)&&(n==2)) return r[4]/14.*(x[2]-1)*(-1+7*r[1]*x[1]-6*x[2]);
	if ((m==2)&&(n==2)) return r[3]/14.*(r[2]*x[1]*(13-41*x[2])-4*(x[1]+6*x[3])+r[1]*(5+9*x[2]+42*x[4]));
	if ((m==2)&&(n==3)) return r[4]/14.*(x[2]-1)*(-1+7*r[1]*x[1]-6*x[2]);
...
{
	histo_t logr;
	if (my_abs(r[1] - 1)<EPS) logr = 0.;
	else logr = my_log(my_abs((r[1] + 1)/(r[1] - 1)));
	
	if ((m==1)&&(n==1)) return -1./84./r[1]*(2*r[1]*(19-24*r[2]+9*r[4])-9*my_pow(r[2]-1,3)*logr);
	if ((m==1)&&(n==2)) return 1./112./r[3]*(2*r[1]*(r[2]+1)*(3-14*r[2]+3*r[4])-3*my_pow(r[2]-1,4)*logr);
	if ((m==2)&&(n==2)) return 1./336./r[3]*(2*r[1]*(9-185*r[2]+159*r[4]-63*r[6])+9*my_pow(r[2]-1,3)*(7*r[2]+1)*logr);
	if ((m==2)&&(n==3)) return 1./112./r[3]*(2*r[1]*(r[2]+1)*(3-14*r[2]+3*r[4])-3*my_pow(r[2]-1,4)*logr);
...
histo_t B_mat(size_t n,size_t a,size_t b,histo_t* r,histo_t* x)
{
	...
	if ((n==2)&&(a==1)&&(b==2)) return -3*r[2]/16.*my_pow(x[2]-1,2)*(6-30*r[1]*x[1]-5*r[2]+35*r[2]*x[2]);
	//n=3
