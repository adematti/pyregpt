...
void set_terms_bias_1loop(size_t nk,histo_t* k,histo_t* pk_lin,histo_t* sigma_v2,histo_t* pk_b2d,histo_t* pk_bs2d,histo_t* pk_b2t,histo_t* pk_bs2t,histo_t* pk_b22,histo_t* pk_b2s2,histo_t* pk_bs22,histo_t* sigma3sq)
{
	terms_bias.nk = nk;
	terms_bias.k = k;
...
#pragma omp for nowait schedule(dynamic)
		for (ik=0;ik<terms_bias.nk;ik++) {
			...
			terms_bias.pk_bs2d[ik] = calc_pkcorr_from_bias_1loop(DELTA,k,kernel_bs2,1);
			terms_bias.pk_b2t[ik] = calc_pkcorr_from_bias_1loop(THETA,k,kernel_b2,1);
			terms_bias.pk_bs2t[ik] = calc_pkcorr_from_bias_1loop(THETA,k,kernel_bs2,1);
			terms_bias.pk_b22[ik] = calc_pkcorr_from_bias_1loop(DELTA,k,kernel_b22,0);
			terms_bias.pk_b2s2[ik] = calc_pkcorr_from_bias_1loop(DELTA,k,kernel_b2s2,0);
			terms_bias.pk_bs22[ik] = calc_pkcorr_from_bias_1loop(DELTA,k,kernel_bs22,0);
