...
void set_terms_bias_1loop(size_t nk,histo_t* k,histo_t* pk_lin,histo_t* sigma_v2,histo_t* pkbias_b2d,histo_t* pkbias_bs2d,histo_t* pkbias_b2t,histo_t* pkbias_bs2t,histo_t* pkbias_b22,histo_t* pkbias_b2s2,histo_t* pkbias_bs22,histo_t* sigma3sq)
{
	terms_bias.nk = nk;
	terms_bias.k = k;
...
#endif //_VERBOSE
			terms_bias.pkbias_b2d[ik] = calc_pkcorr_from_bias_1loop(DELTA,k,kernel_b2,1);
			//terms_bias.pkbias_b2d[ik] = calc_pkcorr_from_bias_1loop(DELTA,k,kernel_b2,0);
			terms_bias.pkbias_bs2d[ik] = calc_pkcorr_from_bias_1loop(DELTA,k,kernel_bs2,1);
			terms_bias.pkbias_b2t[ik] = calc_pkcorr_from_bias_1loop(THETA,k,kernel_b2,1);
			terms_bias.pkbias_bs2t[ik] = calc_pkcorr_from_bias_1loop(THETA,k,kernel_bs2,1);
			terms_bias.pkbias_b22[ik] = calc_pkcorr_from_bias_1loop(DELTA,k,kernel_b22,0);
			terms_bias.pkbias_b2s2[ik] = calc_pkcorr_from_bias_1loop(DELTA,k,kernel_b2s2,0);
			terms_bias.pkbias_bs22[ik] = calc_pkcorr_from_bias_1loop(DELTA,k,kernel_bs22,0);
			terms_bias.sigma3sq[ik] = calc_pkcorr_from_bias_1loop(DELTA,k,kernel_sigma3sq,0);
		}
#pragma omp critical
		{
			free_bias_1loop();
